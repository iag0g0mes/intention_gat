...
from utils.preprocessing import load_data, preprocessing, print_shape, onehot_maneuver
...
def load_kfold_data(
    ...
    x_train, y_train, seqs_train = preprocessing(train_data, input_shape, return_sequence)
...
def cut_by_threshold(
    ...
    y_lat = y[0]
    y_lat_max = np.max(y_lat, axis=1)
    idx_y_lat = np.where(y_lat_max>=threshold)[0]

    new_y_lat = y[0][idx_y_lat, :]
    new_y_lon = y[1][idx_y_lat, :]
    new_seqs  = seqs[idx_y_lat]
    
    traj = x[0][idx_y_lat, :]
    lane = x[1][idx_y_lat, :]
    dev  = x[2][idx_y_lat, :]
    edge = [x[3][i] for i in idx_y_lat]
    nodes = [x[4][i] for i in idx_y_lat]
    
    new_x = [traj, lane, dev, edge, nodes]
    new_y = [new_y_lat, new_y_lon]
    
    return (new_x, new_y, new_seqs)
     

if __name__ == "__main__":
    
    print("\033[94m[Intention Prediction][Graph Maneuver Prediction][main]\033[0m creating model...")
    ...
    print("\033[94m[Intention Prediction][Graph Maneuver Prediction][main]\033[0m loading data...")
    ...
    threshold = config['PARAMS'].getfloat('threshold')
    ...
    #load data
    train_data = load_data(train_dir, size=size)
    x_train, y_train, seqs_train = preprocessing(train_data, input_shape, return_sequence=True)
    y_train[0] = onehot_maneuver(y_train[0])
    y_train[1] = onehot_maneuver(y_train[1])

    # x_train, y_train, seqs_train = cut_by_threshold(x_train, y_train, seqs_train, threshold)
    
    x_train, y_train=\
        data_augmentation(
            X=x_train,
            Y=y_train,
            operations=['conti_noise', 'inter_noise', 'spike_noise'], 
            opt_probs =[0.2, 0.35, 0.3],
            samples_per_class=5500,
        )

    (x_val, y_val, seqs_val),\
    (_, _, _) =\
        load_kfold_data(val_dir, input_shape=input_shape, size=size, return_sequence=True)
    
    print_shape(mode='train', x=x_train, y=y_train)
    print_shape(mode='val', x=x_val, y=y_val)
    
    # print("\033[94m[Intention Prediction][Graph Maneuver Prediction][main]\033[0m applying threshold to train data...")
    ...
    if config['PARAMS']['mode'] == "train":
        print("\033[94m[Intention Prediction][Graph Maneuver Prediction][main]\033[0m training model...")
        
        data_str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        log_dir = "logs/" + data_str
        log_dir = os.path.join(model_dir, log_dir)
        print(f"\033[94m[Intention Prediction][Graph Maneuver Prediction][main]\033[0m[train] log_dir:{log_dir}")
        ...
        #params 
        params_file = os.path.join(log_dir, "intention.ini")
        params_source = os.path.join(FILE.parents[0], "cfg", "intention.ini")
        ...
        print("\033[94m[Intention Prediction][Graph Maneuver Prediction][main]\033[0m testing model...")
